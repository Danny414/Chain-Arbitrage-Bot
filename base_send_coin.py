"""
Standalone script — reads base_bot/hot_log.jsonl and sends a token today summary
to the Base Telegram channel as a bot alert.

Usage:
    python3 base_send_coin.py [SYMBOL]

Default symbol is BRETT. Run any time after the Base Whale Bot has been running
with file persistence enabled (base_bot/hot_log.jsonl must exist).
"""
...
TOKEN   = os.environ.get("BASE_TG_TOKEN", "")
CHAT_ID = os.environ.get("BASE_TG_CHAT_ID", "")
HOT_LOG = "base_bot/hot_log.jsonl"

SYMBOL = sys.argv[1].upper() if len(sys.argv) > 1 else "BRETT"
...
async def main() -> None:
    if not TOKEN or not CHAT_ID:
        print("BASE_TG_TOKEN / BASE_TG_CHAT_ID not set in environment")
        return
