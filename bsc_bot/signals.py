"""
BSC Whale Bot — Concentrated-flow signal cards.
...
logger = logging.getLogger("bsc_bot.signals")
...
# Only cache confirmed no-pool tokens (HTTP 400) — never cache rate-limit failures.
...
async def _fetch_ohlcv(session, pool: str, aggregate: int, limit: int) -> list | None:
    try:
        ...
        logger.debug(f"[Signal] OHLCV failed for pool {pool[:10]}…: {exc}")
        return None


def _confidence_and_sizing(direction: str, rsi_4h, rsi_1h, vol_ratio, vwap_above) -> tuple[str, float, float, float]:
    """
    Returns (confidence_label, sl_pct, tp1_pct, tp2_pct).
    Larger SL/TP when confidence is high (more aggressive trade).
    """
...
async def build_signal_card(
    ...
    total_usd:      float,
    total_amount:   float,
    cex_names:      list[str], # exchange names involved, e.g. ["Binance", "Bybit"]
    window_mins:    int,
) -> str | None:
    """
    Build a concentrated-flow signal card. Always returns a card if price > 0
    (with N/A for TA indicators when no DEX pool is found).
    """
    if price <= 0:
        return None
    ...
    vol_str   = f"{vol_ratio:.1f}x avg" if vol_ratio is not None else "N/A"
    ...
    lines = [
        f"{header_emoji} <b>{title_label} — {symbol}</b> | BSC",
        "━━━━━━━━━━━━━━━━━━━━",
        "",
